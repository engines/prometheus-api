...
require 'loki/api/client'
require 'loki/api/types'

puts "##############################################################"
puts
puts "This is ~a bit~ very shit as it requires a running loki server"
puts
puts "##############################################################"
...
uri = Addressable::URI.parse("http://localhost:3100")

describe Loki::API::Client do
  describe "#query" do
    it "execute a snapshot query returning the lastest value" do
      ...
      client = Loki::API::Client.new(uri)
    ...
    it "execute a snapshot query returning the value for the specified time" do
      ...
      res = Loki::API::Client.new(uri).query(SnapshotQuery.new(:query => query, :time => timestamp))
    ...
    it "execute a range query returning the value for the specified interval" do
      ...
      res = Loki::API::Client.new(uri).query(query)
